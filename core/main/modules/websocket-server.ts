...
type Event = { type: 'log'; level: 'info' | 'error'; message: string } | { type: 'status'; status: Status };
...
export function register(): void {
  ...
  const publishStatus = () => {
    sendEvent({
      type: 'status',
      status: buildStatus(Boolean(server && wss), currentUrl, currentTls, clients, totalRecv, totalSent),
    });
  };
