...
function parseParams(params: unknown): { ok: true; data: MqttConnParams } | { ok: false; error: string } {
  ...
  if (!id || !protocol || !host || !port || !clientId)
    return { ok: false, error: 'invalid mqtt connect params' };
...
export function register(): void {
  ...
  ipcMain.handle('mqtt:connect', async (_event, params: unknown) => {
    ...
    const opts: ClientOptionsExt = {
      ...
      reconnectPeriod:
        p.autoReconnect === false ? 0 : Math.max(0, Math.min(60000, Number(p.reconnectPeriod ?? 4000))),
      ...
      try {
        ...
        if (hasV4 && hasV6) {
          ...
          opts.lookup = (hostname: string, options: unknown, cb?: unknown) => {
            ...
            if (wantsAll) {
              const nextOptions = (
                isRecord(rawOptions) ? { ...rawOptions, all: true, family: 0 } : { all: true, family: 0 }
              ) as dnsCb.LookupAllOptions;
              const cbAll = callback as (
                err: NodeJS.ErrnoException | null,
                addresses: dnsCb.LookupAddress[],
              ) => void;
            ...
            const nextOptions = (
              isRecord(rawOptions) ? { ...rawOptions, family: 4, all: false } : { family: 4, all: false }
            ) as dnsCb.LookupOneOptions;
            const cbOne = callback as (
              err: NodeJS.ErrnoException | null,
              address: string,
              family: number,
            ) => void;
  ...
  ipcMain.handle(
    'mqtt:publish',
    (_event, id: string, topic: string, payload: string, qos: number, retain: boolean) => {
      const c = clients.get(String(id ?? ''))?.client;
      if (!c?.connected) return err('io_error', 'MQTT client not connected');
      try {
        c.publish(String(topic ?? ''), String(payload ?? ''), {
          qos: (Number(qos) as 0 | 1 | 2) ?? 0,
